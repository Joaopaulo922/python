...
class Funcs():
    ...
    def add_cliente(self):
        ...
        if self.nome_entry.get() == "":
           ...
           messagebox.showinfo("Cadastro de clientes - Aviso!!!", msg)
            ...
            self.cursor.execute(""" INSERT INTO clientes (nome_cliente, telefone, cidade, endereco, debito, sexo, cpf)
            VALUES (?, ?, ?, ?, ?, ?, ?)""", (self.nome, self.telefone, self.cidade, self.endereco, self.debito, self.sexo, self.cpf))
            self.com.commit()
            self.desconecta_bd()
            self.select_lista()
            self.limpa_tela()
    ...
    def select_lista(self):
        ...
        cidade, endereco, debito, sexo, cpf FROM clientes ORDER BY nome_cliente ASC; """)
        for i in lista:
    ...
    def onDoubleClick(self, event):
        ...
        for n in self.listaCli.selection():
            ...
            self.sexo_entry.insert(END, col7)
            self.cpf_entry.insert(END, col8)
    ...
    def alterar_cliente(self):
        ...
        self.variaveis()
        self.conecta_bd()
        self.cursor.execute(""" UPDATE clientes SET nome_cliente = ?, telefone = ?, cidade = ?, endereco = ? , debito = ?, sexo = ?, cpf = ? WHERE cod = ? """,
                            (self.nome, self.telefone, self.cidade, self.endereco, self.debito, self.codigo, self.sexo, self.cpf))
        self.com.commit()
        self.desconecta_bd()
        self.select_lista()
        self.limpa_tela()
    ...
    def buscar_cliente(self):
        ...
        self.cursor.execute(
            """ SELECT cod, nome_cliente, telefone, cidade, endereco, debito, sexo, cpf FROM clientes
    ...
    def buscar_produto(self):
        ...
        #listaprod
        #nome_produtoVenda_entry
    def buscar_produtoTV(self):
        self.conecta_bd_P()
        self.listaprod.delete(*self.listaprod.get_children())

        self.nome_produtoVenda_entry.insert(END, '%')
        nome = self.nome_produtoVenda_entry.get()
        self.cursor.execute(
            """ SELECT cod, nome_produto, tamanho, marca, valor FROM estoque
            WHERE nome_produto LIKE '%s' ORDER BY nome_produto ASC""" % nome)
        buscanomePro = self.cursor.fetchall()
        ...
        self.limpa_telaProdutos()
        self.desconecta_bd()
    def buscar_notaFiscal(self):
        baixarnota = webbrowser.open('https://www.google.com')
    def buscar_clientePV(self):
        self.conecta_bd(); print("Conentacndo ao banco de dados")
        self.verClientes.delete(*self.verClientes.get_children())

        self.pCliente_entry.insert(END, '%')
        nome = self.pCliente_entry.get()
        self.cursor.execute(
            """ SELECT cod, nome_cliente, telefone, cidade, endereco, debito, sexo, cpf FROM clientes
            WHERE nome_cliente LIKE '%s' ORDER BY nome_cliente ASC""" % nome)
        buscanomeCli2 = self.cursor.fetchall()
        for i in buscanomeCli2:
            self.verClientes.insert("", END, values=i)
...
class Application(Funcs, Relatorios, Validadores):
    ...
    def widgets_frame_1(self):
        ...
        # entrada de CPF
        self.lb_cpf = Label(self.aba1, text="Cpf", fg="black", bg=self.cor_nova)
        self.lb_cpf.place(relx=0.58, rely=0.35)
        self.cpf_entry = Entry(self.aba1, bg=self.corTela, validate="key", validatecommand=self.vcmd2)
        self.cpf_entry.place(relx=0.58, rely=0.45, relwidth=0.1, relheight=0.08)
        # entrada de sexo
        self.lb_sexo = Label(self.aba1, text="Sexo", fg="black", bg=self.cor_nova)
        self.lb_sexo.place(relx=0.88, rely=0.35)
        self.sexo_entry = Entry(self.aba1, bg=self.corTela)
        self.sexo_entry.place(relx=0.88, rely=0.45, relwidth=0.1, relheight=0.08)
        ...
        self.lb_debito.place(relx=0.72, rely=0.35)
        self.debito_entry = Entry(self.aba1, bg=self.corTela,  validate="key", validatecommand=self.vcmd4)
        self.debito_entry.place(relx=0.72, rely=0.45, relwidth=0.1)
        ...
        self.popupMenu.place(relx=0.42, rely=0.01, relwidth=0.13, relheight=0.15)
        self.estado_civil = self.Tipvar.get()

        ## criando calendario
        self.bt_calendario = Button(self.aba3, text="Data", command=self.Calendario)
        self.bt_calendario.place(relx=0.42, rely=0.2)
        self.entry_data = Entry(self.aba3, width=10)
        self.entry_data.place(relx=0.42, rely=0.3)
        ...
        self.preco_entry = Entry(self.aba2, bg=self.corTela)
        ...
        ToolTip(self.bt_vender, msg='Abrir tela de venda')
        ToolTip(self.bt_vender2, msg='Abrir tela de venda')

        # OBJETOS DA ABA3
        self.bt_notasF = Button(self.aba3, text="Nota Fiscal", bd=3, fg="white",
                                activebackground=self.azul2, activeforeground='white',
                                font=("verdana", 8, "bold"), bg=self.azul, command=self.buscar_notaFiscal)
        self.bt_notasF.place(relx=0.01, rely=0.02, relwidth=0.15, relheight=0.15)
        ToolTip(self.bt_notasF, msg="Buscar nota fiscal")
    # LISTAS FRAME 2
    def lista_frame2(self):
        ...
        self.listaCli.heading("#7", text="Sexo")
        self.listaCli.heading("#8", text="Cpf")
        ...
        self.listaCli.column("#8", width=120)
    ...
    def janela2(self):
        ...
        self.bt_pClientes = tk.Button(self.frame_4, text="Procurar", bd=3, fg="white",
                                                activebackground=self.azul, activeforeground='white',
                                                font=("verdana", 8, "bold"), bg=self.azul2, command=self.buscar_clientePV)
        self.bt_pClientes.place(relx=0.60, rely=0.02, relwidth=0.15, relheight=0.06)
        ...
        self.verClientes.heading("#6", text="Sexo")
        self.verClientes.heading("#7", text="Cpf")
        ...
        self.verClientes.column("#6", width=50)
        self.verClientes.column("#7", width=50)
    ...
    def janela3(self):
        ...
        self.frame_7 = Frame(tela_venda,  highlightbackground=self.cinza, highlightthickness=3)
        self.frame_7.place(relx=0.72, rely=0.40, relwidth=0.3, relheight=0.3)
        # botao vender
        ...
        self.bt_produrar_Produto = tk.Button(self.frame_3, text="Produrar produto", bd=3, fg="white",
                                   activebackground=self.vermelho2, activeforeground='white',
                                   font=("verdana", 8, "bold"), bg=self.azul, command=self.buscar_produtoTV)
        ...
        self.lb_totalvenda = Label(self.frame_7, text="Total", fg="black", bg=self.cinza)
        self.lb_totalvenda.place(relx=0.45, rely=0.1,relwidth=0.3)
        ...
        self.listaprod.heading("#4", text="Marca")
